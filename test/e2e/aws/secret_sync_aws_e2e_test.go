...
func TestOpenBaoPluginSyncsToAWSSecretsManager(t *testing.T) {
	...
	writeAWSDestination(t, baoClient)
...
func TestCleanupAWSSecrets(t *testing.T) {
	if os.Getenv("E2E_AWS_CLEAN_CONFIRM") != "1" {
		t.Skip("set E2E_AWS_CLEAN_CONFIRM=1 to force-delete manual AWS e2e secrets")
	}
	...
	if err != nil {
		t.Fatalf("cleanup AWS secrets with prefix %q: %v", prefix, err)
	}
	t.Logf("force-deleted %d AWS Secrets Manager secrets under %q", deleted, prefix)
}

func requireManualAWS(t *testing.T) {
	t.Helper()
	if os.Getenv("E2E_AWS_CONFIRM") != "1" {
		t.Skip("set E2E_AWS_CONFIRM=1 to run the manual AWS e2e test")
	}
}
...
func newOpenBaoClient(t *testing.T) *api.Client {
	...
	config.Address = env("E2E_AWS_OPENBAO_ADDR", "http://127.0.0.1:18201")
	...
	waitFor(t, ctx, func() error {
		...
		for key, expected := range expectedTags {
			if got := tags[key]; got != expected {
				return fmt.Errorf("tag %s = %q, want %q", key, got, expected)
			}
		}
		return nil
	})
...
func cleanupAWSSecrets(ctx context.Context, client *secretsmanager.Client, prefix string) (int, error) {
	...
	for paginator.HasMorePages() {
		...
		for _, secret := range page.SecretList {
			...
			if err := forceDeleteSecret(ctx, client, name); err != nil {
				return deleted, err
			}
			deleted++
		}
...
func forceDeleteSecret(ctx context.Context, client *secretsmanager.Client, secretName string) error {
	...
	if _, restoreErr := client.RestoreSecret(ctx, &secretsmanager.RestoreSecretInput{
		SecretId: aws.String(secretName),
	}); restoreErr != nil && !isNotFound(restoreErr) {
		return restoreErr
	}
	_, err = client.DeleteSecret(ctx, &secretsmanager.DeleteSecretInput{
		SecretId:                   aws.String(secretName),
...
func awsRegion(t *testing.T) string {
	t.Helper()
	return env("E2E_AWS_REGION", env("AWS_REGION", defaultAWSRegion))
}
